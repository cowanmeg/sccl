...
class PathEncodingBase(object):
            ...
            for step in range(instance.steps - overlap):
                ...
                for src in srcs:
                    for dst in dsts:
                        ...
                        pbsends.extend([(Or(sends),1) for sends in sends_by_addr.values()])
                ...
                for i in range(1, instance.extra_rounds + 2):
                    s.add(Implies(_rounds(step) == i, PbLe(pbsends, bw * i)))
    ...
    def solve(self, instance):
        ...
        if self.heuristic is not None:
            self.heuristic.add_constraints(self.collective, self.topology, solver)
        if solver.check() == sat:
            model = solver.model()
